...
use anyhow::Context;
use postject_sys::postjector::postjector_buffer_free;
...
struct OwnedPostInjectorBuffer(PostjectorBuffer);
impl OwnedPostInjectorBuffer {
    pub unsafe fn new(raw: PostjectorBuffer) -> Self {
        Self(raw)
    }
}
impl Drop for OwnedPostInjectorBuffer {
    fn drop(&mut self) {
        unsafe { postjector_buffer_free(self.0) }
    }
}
impl Deref for OwnedPostInjectorBuffer {
    ...
    fn deref(&self) -> &Self::Target {
        unsafe { std::slice::from_raw_parts(self.0.head, self.0.size) }
    }
}
...
pub fn inject(
    ...
    unsafe {
        ...
        if inject_result.type_ == POSTJECTOR_INJECT_ALREADY_EXISTS {
            ...
            let output = OwnedPostInjectorBuffer::new(inject_result.data);
